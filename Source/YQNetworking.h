 ...
 */
typedef void (^YQUploadProgress)(int64_t bytesWritten,
                                 int64_t totalBytesWritten);

typedef NS_ENUM(NSUInteger, YQResponseType) {
    kYQResponseTypeJSON = 1, // 默认
    kYQResponseTypeXML  = 2, // XML
    // 特殊情况下，一转换服务器就无法识别的，默认会尝试转换成JSON，若失败则需要自己去转换
    kYQResponseTypeData = 3
};

typedef NS_ENUM(NSUInteger, YQRequestType) {
    kYQRequestTypeJSON = 1, // 默认
    kYQRequestTypePlainText  = 2 // 普通text/html
};

typedef NS_ENUM(NSInteger, YQNetworkStatus) {
    kYQNetworkStatusUnknown          = -1,//未知网络
    kYQNetworkStatusNotReachable     = 0,//网络无连接
    kYQNetworkStatusReachableViaWWAN = 1,//2，3，4G网络
    kYQNetworkStatusReachableViaWiFi = 2,//WIFI网络
};
...
typedef void(^YQResponseSuccess)(id response);
typedef void(^YQResponseFail)(NSError *error);
 ...
 *  @param isAutoEncode YES or NO,默认为NO，是否自动encode url
 *  @param isCallbackOnCancelRequest 当取消请求时，是否要回调，默认为YES
 */
 ...
 *    @author QuinceyYang
 *
 *    取消某个请求。如果是要取消某个请求，最好是引用接口所返回来的YQURLSessionTask对象，
 ...
 *  @author QuinceyYang,
 *
 *  POST请求接口，支持下载进度，若不指定baseurl，可传完整的url。
 *
 *  @param url     接口路径，如/path/getArticleList
 *  @param params  接口中所需的参数，如@{"categoryid" : @(12)}
